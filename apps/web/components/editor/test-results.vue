...
<template>
  <div class="rounded-lg border border-rule bg-muted/50 p-4">
    ...
    <div v-if="results && results.length > 0" class="space-y-3">
      <div v-for="(result, index) in results" :key="index" class="text-sm">
        <div class="flex items-center gap-2">
          <span :class="result.passed ? 'text-success' : 'text-destructive'" class="flex-shrink-0">
            {{ result.passed ? '✓' : '✗' }}
          </span>
          <span
            :class="result.passed ? 'text-foreground' : 'text-destructive'"
            class="flex-1 truncate"
          >
            {{ result.name }}
          </span>
          <span v-if="result.duration != null" class="text-muted-foreground flex-shrink-0"
            >{{ result.duration.toFixed(1) }}ms</span
          >
        </div>
        <div v-if="result.message && !result.passed" class="mt-1 ml-5">
          <pre
            class="text-xs text-muted-foreground bg-muted p-2 rounded overflow-x-auto max-h-40 whitespace-pre-wrap break-words"
            >{{ formatMessage(result.message) }}</pre>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="status === 'error'" class="space-y-2">
      ...
      <pre
        v-if="errorMessage"
        class="text-xs bg-destructive/10 text-destructive p-3 rounded-md overflow-x-auto max-h-40 whitespace-pre-wrap break-words"
        >{{ formatMessage(errorMessage) }}</pre>
