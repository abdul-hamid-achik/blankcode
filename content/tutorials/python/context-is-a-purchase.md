...
slug: "python-context-is-a-purchase"
...
::code-blank{lang="python" href="/tracks/python/context" label="practice give it what it needs for real"}
---
code: |
  await client.request(
      "___blank_start___POST___blank_end___",
      f"/api/orders/{order_id}/___blank_start___cancel___blank_end___",
  )
---
::
...
::code-blank{lang="sql" href="/tracks/python/context" label="practice give it what it needs for real"}
---
code: |
...
The second cost is quality. Irrelevant context is not neutral padding; it is material the model will try to use. Hand over the SQLAlchemy docs alongside a request for raw SQL and the answer drifts toward the manual's idioms. Hand over twenty example rows and a column that happens to be null in all of them reads as optional — the rows show what happens to be there, while the definitions state what can be there. Every unnecessary source is another surface the model can anchor on, and it does not tell you when it has.

## Where this bites

**Pasting the whole module to ask about one function.** The rest of the file is anchor material, and it is billed on every message of the conversation that follows. Excerpt the function and the types it touches; that is the local delta, and it is small.
