...
slug: "vue-reviewing-generated-code"
...
The choice arrives wearing the same confident prose as everything else. That is what makes this class of review hard: the wrong reading of your spec is rendered as fluently as the right one, with a comment asserting the behavior you wanted, above code that does something else. So skip the style read entirely. List the decisions the spec forced — boundary behavior, error behavior, ordering, concurrency — and check which way each one actually went.
...
The suite passes anyway, because every total it tries divides evenly by the page size: 30 records, 10 per page. The same misreading that wrote `floor` picked the fixtures — a suite born from the bug cannot see the bug.

::code-blank{lang="typescript" href="/tracks/vue/code-review" label="practice code review for real"}
---
code: |
...
Then take the smallest input from an unvisited class and trace it by hand. Twenty-five records, page size 10: `total / pageSize` is 2.5, `floor` makes it 2, the loop fetches page 1 and stops. Thirty seconds of arithmetic finds what an hour of reading the loop for "off-by-one vibes" does not.
...
The suite passed for two reasons. First, it asserted that `save` was *called*. Second, its mock resolved instantly, so the pushes squeeze in before the caller looks. Give `save` one real timer tick and the same caller receives 0 items — and the same array holds 3 a few milliseconds later. A mock does not just fake data; it fakes *timing*.

The fix is not a heavier mock. It is a loop the language can await:
...
**Trusting a suite whose fixtures are all round numbers.** Totals that divide evenly, mocks that resolve instantly, inputs that never fail — each one is a class of behavior the green checkmark says nothing about.

**Accepting a fix that silences the symptom.** When generated error handling swallows a failure and a later, louder failure gets reported instead, guarding the loud one makes the report quieter and the diagnosis worse.

**Asking the model to review its own output in the same conversation.** It re-reads the code with the same reading of the spec that produced it — the one thing a reviewer is there to not share. Fresh review needs fresh context.
